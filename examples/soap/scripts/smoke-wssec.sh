...
echo "==> Waiting for Velonetics gateway (WS-Security config)"
for i in $(seq 1 90); do
  if curl -fsS "http://127.0.0.1:8080/secure/country/US" >/dev/null 2>&1; then
    break
  fi
  sleep 2
  if [[ "$i" -eq 90 ]]; then
...
echo "==> WS-Security UsernameToken smoke test"
RESP="$(curl -fsS "http://127.0.0.1:8080/secure/country/US")"
echo "$RESP"
...
echo "All SOAP WS-Security smoke checks passed."
